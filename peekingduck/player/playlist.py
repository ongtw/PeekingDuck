...
class PlayList:
    ...
    def __contains__(self, item):
        res = str(item) in self._pipeline_list
    ...
    def add_pipeline(self, pipeline_path: Union[Path, str]) -> None:
        ...
        """
        self._pipeline_list.append(str(pipeline_path))
    ...
    def remove_pipeline(self, pipeline_path: Union[Path, str]) -> None:
        ...
        """
        self._pipeline_list.remove(str(pipeline_path))
    ...
    def _verify_pipeline_files(self) -> None:
        ...
        for pipeline in self._pipeline_list:
            is_exist = False  # file exist flag
            mod_datetime = None  # last modified date/time

            pipeline_path_obj = Path(pipeline)
            if pipeline_path_obj.exists():
                is_exist = True
                mod_time = pipeline_path_obj.stat().st_mtime
